...
function ElkBuffBars:OnInitialize()
	...
	if not self.db.global.build or self.db.global.build ~= build or true then
		self.db.global.build = build
		for k, t in pairs(self.db.global.maxtimes) do wipe(t) end
		for k, t in pairs(self.db.global.maxcharges) do wipe(t) end
	end
...
--~ 	end
	...
	self.bucket_PLAYER_FOCUS_CHANGED = self:RegisterBucketEvent("PLAYER_FOCUS_CHANGED", .1)
...
function ElkBuffBars:AddBarGroup(layout)
	...
	if not AO_groupsettings[settigsId] then
...
function ElkBuffBars:ScanData_UnitAura(unit, auratype)
	...
	local i = 1
	while true do
		--    name, rank, texture, count, debuffType, duration, expirationTime, unitCaster, canStealOrPurge, nameplateShowPersonal, spellId, canApplyAura, isBossDebuff, isCastByPlayer, nameplateShowAll, timeMod...
		local name, rank, texture, count, debuffType, duration, expirationTime, unitCaster, canStealOrPurge, nameplateShowPersonal, spellId, canApplyAura, isBossDebuff, isCastByPlayer, nameplateShowAll, timeMod = UnitAura(unit, i, filter)
...
function ElkBuffBars:GetTempBuffName(slot, enchantId)
	...
	enchantName = select(3, Gratuity:Find("^(.+) %(%d+ [^%)]+%)$")) -- removes 1st bracket (time left / charges)
	if enchantName then
		...
		if tname then
			...
			local tname, trank = string_match(enchantName, "^(.*) ([CDILMVX]+)$")
			if tname then
				enchantName = tname
